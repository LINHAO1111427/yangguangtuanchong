...
public class AppTradeOrderController {
    ...
    @Operation(summary = "获得订单结算信息（POST）", description = "用于前端使用 JSON body 提交复杂的 items 参数")
    public CommonResult<AppTradeOrderSettlementRespVO> settlementOrderByPost(
            @Valid @RequestBody AppTradeOrderSettlementReqVO settlementReqVO) {
        return success(tradeOrderUpdateService.settlementOrder(getLoginUserId(), settlementReqVO));
    }
