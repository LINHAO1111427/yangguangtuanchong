...
public class MemberUserServiceImpl implements MemberUserService {
    ...
    @Override
    public List<MemberUserDO> searchUsers(String keyword, int limit) {
        return memberUserMapper.searchUsers(keyword, limit);
    }

    @Override
    public MemberUserDO getUserByCustomId(Long customId) {
        if (customId == null) {
            return null;
        }
        return memberUserMapper.selectByCustomId(customId);
    }
    ...
    public void updateUser(Long userId, AppMemberUserUpdateReqVO reqVO) {
        MemberUserDO updateObj = new MemberUserDO()
                ...
                .setSex(reqVO.getSex())
                .setMark(reqVO.getMark())
                .setBackgroundUrl(reqVO.getBackgroundUrl())
                .setRegion(reqVO.getRegion())
                .setOccupation(reqVO.getOccupation())
                .setSchool(reqVO.getSchool());
        if (StrUtil.isNotBlank(reqVO.getBirthday())) {
            try {
                LocalDate birthday = LocalDate.parse(reqVO.getBirthday(), DateTimeFormatter.ISO_LOCAL_DATE);
                updateObj.setBirthday(birthday.atStartOfDay());
            } catch (Exception e) {
