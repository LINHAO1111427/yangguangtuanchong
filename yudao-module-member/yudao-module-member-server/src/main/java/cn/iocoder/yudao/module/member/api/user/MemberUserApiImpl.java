...
public class MemberUserApiImpl implements MemberUserApi {
    ...
    @Override
    public CommonResult<List<MemberUserRespDTO>> searchUsers(String keyword, Integer limit) {
        int queryLimit = limit == null ? 20 : limit;
        List<MemberUserDO> users = userService.searchUsers(keyword, queryLimit);
    ...
    // 作者：Lin - 新增：基于 custom_id 精确匹配用户
    @Override
    public CommonResult<MemberUserRespDTO> getUserByCustomId(Long customId) {
        MemberUserDO user = userService.getUserByCustomId(customId);
        return success(MemberUserConvert.INSTANCE.convert2(user));
    }
